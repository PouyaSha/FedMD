...
if __name__ == "__main__":
    ...
    if model_saved_dir is None:
        ...
        for q in range(5):
            print("round ",q)
            unlabeled_data_y_train = []
            unlabeled_data_y_test = []

            for n, model in enumerate(parties):
                unlabeled_data_y_train.append(model.predict(X_train_MNIST))
                unlabeled_data_y_test.append(model.predict(X_test_MNIST))
            
            unlabeled_data_y_train = np.array(unlabeled_data_y_train).argmax(axis=2)
            unlabeled_data_y_test = np.array(unlabeled_data_y_test).argmax(axis=2)


            pre_train_result = train_models(parties,
                                            X_train_MNIST, unlabeled_data_y_train
                                            ,X_test_MNIST, unlabeled_data_y_test,
